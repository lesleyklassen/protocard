...
def reset_deck(original_deck, deck, discard_pile, hand=None):
    ...
    if hand is not None:
        hand.clear()
    shuffle_deck(deck)  # Shuffle the deck after resetting
    console.print("[bold red]Deck reset to original state. Cards in deck: {}[/bold red]".format(len(deck)))

def initial_shuffle(deck):
    random.shuffle(deck)
    console.print("[bold green]Initial shuffle completed. Cards in deck: {}[/bold green]".format(len(deck)))
...
def main():
    ...
    initial_shuffle(deck)  # Initial shuffle after loading the deck

    while True:
        console.print("[bold blue]Choose deck type:[/bold blue]\n1. Draw and Play\n2. Draw and Hold\n3. Quit\n")
        deck_type = input("Enter the number corresponding to the deck type: ").strip()
        if deck_type == "3":
            console.print("[bold yellow]Quitting the game. Goodbye![/bold yellow]")
            break

        deck = original_deck.copy()
        discard_pile.clear()
        shuffle_deck(deck)  # Shuffle the deck every time it is used
